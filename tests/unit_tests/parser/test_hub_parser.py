...
def test_parse_valid_start_hub() -> None:
    ...
    raw_hub: RawHub = parse_hub(start_content, start_type, 42, 5)
...
def test_parse_valid_end_hub() -> None:
    ...
    raw_hub: RawHub = parse_hub(end_content, end_type, 42, 5)
...
def test_parse_valid_common_hub() -> None:
    ...
    raw_hub: RawHub = parse_hub(common_content, common_type, 42, 2)
...
def test_parse_hub_empty_metadata() -> None:
    raw = parse_hub("hub 0 0 []", "common_hub", 42, 1)
    assert raw.zone == "normal"
    assert raw.color == "default"
    assert raw.max_drones == 1


def test_parse_invalid_hub() -> None:
    with pytest.raises(InvalidSyntaxError):
        parse_hub("corridorA A B [color=green 2]", "common_hub", 42, 1)


def test_parse_invalid_metadata_hub() -> None:
    with pytest.raises(InvalidSyntaxError):
        parse_hub("hub 0 0 [invalid=value]", "common_hub", 42, 1)


def test_hub_name_cannot_contain_dash() -> None:
    with pytest.raises(InvalidSyntaxError):
        parse_hub("hub-A 0 2", "hub", 42, 1)
